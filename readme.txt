...
в приложении практически не используется Model binding и Model validation.
----------------------------------------------------------------------------------------------------------------------------
...
Выполненные необязательные требования:
	...
	!ВАЖНО:	по-хорошему такого рода export надо реализовывать следующим образом (это не сложно, но требует времени):
		...
		по готовности файла пользователю высылается письмо со ссылкой на данный файл (через SMTP или свой почтовый сервис);
